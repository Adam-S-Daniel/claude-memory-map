...
These two workflows (plus `scripts/sync-to-site.sh` / `scripts/upsert-comment.sh`) are the **reference implementation for any tool repo** that vendors onto the site's `/tools/` section — a new tool's repo copies them and changes the slug/paths; the branch conventions generalize to `tool-sync/<slug>` and `tool-preview/<slug>-pr-<n>` (the site side is documented in its embeddable-tool-pages skill and AGENTS.md).

From the next cms-platform release, the site's visual-regression gate auto-passes any tool-sync PR — from any tool repo — that only brings an existing tool's vendored asset (+ provenance record) up to date with its source repo. That's by design, not a gap: the substantive review already happened in the tool's own repo — its PR and preview mirror. A PR that *adds a new tool* still gets the full regression review (its `_tools/<slug>.md` entry is salient).
